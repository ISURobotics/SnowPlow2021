...
#down, back into the "garage". Essentially taking a clockwise path around the entire snowpath.
#
#The snowfield is represented by a 2D array which is is 28 x 56 with each square representing 0.25m x 0.25m in real life.
#
#
#For this competition, there will be two cones randomly placed within the field, one in the snowpath and one outside of the snowpath
#Since the teams robot is roughly 1.2m x 1.4m will only be represented by one spot on the grid, all cones will be represented by a 5x5 section of the
#snowfield, meaning the they will "take up" 1.25m x 1.25m.
#
...
#
#To actaully path find, Dijkstra's Algorithm is used. This algorithm finds the shortest path between two points by implementing
#breadth first search on a weighted 2D array (where the value at each position in the array represents the "weight" or distance of that point.
#the higher the weight, the more it "costs" to travel through that point)
#
#Since the start point and end point are right next to each other, the path will be generated by connecting smaller paths
#found by the path finder. Ex - find path going up, then add onto that path by path finding to the left, etc.
...
#the algorithm will instead search for a point around the predefined endpoint. It will search in a specific direction (defined later) epending
#on which direction the path is supposed to be taking (up, down, left, right). Only searching in one specific direction allows the
#pathfinder to work, however, it does cause some "irregular" paths
#
...
#returns  array of tuples that represnet points of a 2 block (0.5m) buffer around the position of the cone
#
...
# this competition, there is a prefered side-to-side/perpendicular movement as defined below. To accomplish this, a large weight
# of 100 is place in the opposite of the prefered direction in order to deter movement in that direction past where the start point is
#
# DIRECTION     PREFERED SIDE-TO-SIDE MOVEMENT
# Up            Right
# Down          Left
# Left          Down
# Right         Up
...
def path_finder(grid, start, end):
    ...
    # creates a copy of the grid as to not change the inputed grid
...
#return  inputed grid except with 0s placed where cones are
#
...
#return  altered path with removed conseccutive repeats
#
